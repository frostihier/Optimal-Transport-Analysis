...
class TSinkhornSolverStandard:
    ...
    def __init__(self,c,muX,muY,errorGoal,rhoX=None,rhoY=None,alpha=None,beta=None,\
                eps=None,epsInit=None,epsSteps=None,epsList=None,epsStep=0.5,verbose=False):
            ...
            if eps is not None:
                if epsInit is None:
                    ...
                    if epsSteps is None:
                        ...
                        epsSteps=int((np.log(epsInit)-np.log(eps))/np.log(1./epsStep)+1)
...
def SolveW2(muX,posX,muY,posY,
    ...
    SinkhornSolver=TSinkhornSolverStandard(c,muX,muY,SinkhornError,eps=epsTarget,epsInit=epsInit,epsStep=epsStep,verbose=verbose)
...
def SolveHK(muX,posX,muY,posY,HKScale,
    ...
    # create the actual solver object
    SinkhornSolver=TSinkhornSolverKLMarginals(c,muX,muY,HKScale**2,SinkhornError,eps=epsTarget,epsInit=epsInit)
    msg=SinkhornSolver.solve()
    if msg!=0: raise ValueError("solve: {:d}".format(msg))
